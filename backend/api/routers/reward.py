...
async def get_rewards(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    rewards = db.execute(text("SELECT * FROM rewards_reward")).fetchall()
...
async def claim_reward(reward_id: int, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    reward = db.execute(text("SELECT * FROM rewards_reward WHERE id = :reward_id"), {"reward_id": reward_id}).fetchone()
    if reward is None:
...
async def get_achievements(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    achievements = db.execute(text("SELECT * FROM rewards_achievement")).fetchall()
    ...
    char_achievements = {}
    for ca in db.execute(text("SELECT * FROM rewards_characterachievement WHERE character_id = :character_id"), {"character_id": character.id}).fetchall():
